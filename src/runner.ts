...
const ensureMigrationsTable = async (db: DBConnection, options: RunnerOption): Promise<void> => {
  try {
    ...
    if (migrationTables && migrationTables.length === 1) {
      ...
      await db.query(
        `CREATE TABLE ${fullTableName} ( ${idColumn} NOT NULL GENERATED BY DEFAULT AS IDENTITY(1, 1) PRIMARY KEY, ${nameColumn} varchar(255) NOT NULL, ${runOnColumn} timestamp NOT NULL)`,
      )
    }
  } catch (err) {
